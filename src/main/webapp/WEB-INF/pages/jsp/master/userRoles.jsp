...
<head>
    ...
    <script language="JavaScript" type="text/javascript">
        function borrar_usuario(){
            if(validaItems(this)){
                if ( confirm("Do You really want to delete the selected User Roles?")) {
        ...
        function validaItems(source) {
            ...
            if(cta==0){
                alert("There are no user roles selected to remove.");
                return false;
            }
            return true;
        }

        function valida(email,unit,proj,ename,uname,pname){
            document.forma.p_email.value = email;
            document.forma.p_unit.value = unit;
            document.forma.p_proj.value = proj;
            document.forma.p_email_name.value = ename;
            document.forma.p_unit_name.value = uname;
            document.forma.p_proj_name.value = pname;
            document.forma.targetname = "";
...
<div id="pcoded" class="pcoded">
    ...
    <div class="pcoded-container navbar-wrapper">
        ...
        <div class="pcoded-main-container">
            <!--start ############################### page-header ###############################-->
            <jstl:set var="jsp_titleFrom" scope="page" value="User Roles List Form"/>
                ...
                <form class="user" role="form" method="post" action="#!" name="forma" >
                    <table id="other" cellspacing="0" width="100%" border="0">
                        ...
                        <td>
                            ...
                            <input name='p_proj' type='hidden' value='' />
                            <input name='p_email_name' type='hidden' value='' />
                            <input name='p_unit_name' type='hidden' value='' />
                            <input name='p_proj_name' type='hidden' value='' />
                    ...
                    >
                        ...
                        <tr>
                            ...
                            <th data-field="id" data-sortable="false">USER EMAIL</th>
                            <th data-field="name" data-sortable="true" data-switchable="true" data-filter-control="input">USER NAME</th>
                            <th data-field="st" data-sortable="true" data-switchable="true" data-filter-control="input">STATUS</th>
                            <th data-field="rt" data-sortable="true" data-switchable="true" data-filter-control="input">ROLE</th>
                            <th data-field="contry" data-sortable="true" data-switchable="true"data-filter-control="select">UNIT NAME</th>
                            <th data-field="pn" data-sortable="true" data-switchable="true"data-filter-control="select">PROJECT NAME</th>
                        ...
                        <jstl:forEach var="x" begin="0" end="${ rq_numFilas -1 }" step="1">
                            ...
                            <tr>
                                ...
                                <td>
                                    <jstl:out value="${ rq_roles.get(x).getMasterUserMuEmail() }">Lost Value</jstl:out>
                                    <input type='hidden' value='<jstl:out value="${ rq_roles.get(x).getMasterUserMuEmail() }"/>'
                                           name='p_cual_e<jstl:out value="${ f }"/>' />
                                    <input type='hidden' value='<jstl:out value="${ rq_roles.get(x).getMasterUserBussinessUnitBuBisCode() }"/>'
                                           name='p_cual_u<jstl:out value="${ f }"/>' />
                                    <input type='hidden' value='<jstl:out value="${ rq_roles.get(x).getProjectPrIdProject() }"/>'
                                           name='p_cual_p<jstl:out value="${ f }"/>' />
                                </td>
                                <td>
                                    <jstl:out value="${ rq_roles.get(x).getUserName() }">Lost Value</jstl:out>
                                </td>
                                <td>
                                    <jstl:choose>
                                        <jstl:when test="${ rq_roles.get(x).getUmStatus() == 'T' }">
                                            Enabled
                                        </jstl:when>
                                        <jstl:otherwise>
                                            Disabled
                                ...
                                <td>
                                    <jstl:choose>
                                        <jstl:when test="${ rq_roles.get(x).getUmRole() == 1 }">Super User</jstl:when>
                                        <jstl:when test="${ rq_roles.get(x).getUmRole() == 2 }">Administrator</jstl:when>
                                        <jstl:when test="${ rq_roles.get(x).getUmRole() == 3 }">User 1</jstl:when>
                                        <jstl:otherwise>
                                            User 2
                                        </jstl:otherwise>
                                    </jstl:choose>
                                ...
                                <td>
                                    <jstl:out value="${ rq_roles.get(x).getUnitName() }">Lost Value</jstl:out>
                                    <%--<input type='hidden' value='<jstl:out value="${ Math.toIntExact(rq_roles.get(x).getId_country()) }"/>'
                                           name='p_cual_p<jstl:out value="${ f }"/>' />&nbsp;--%>
                                </td>
                                <td>
                                    <jstl:out value="${ rq_roles.get(x).getProjName() }">Lost Value</jstl:out>
                                </td>
                                <td>
                                    <a href='#' title="Edit" onClick=valida('${ rq_roles.get(x).getMasterUserMuEmail() }',${ rq_roles.get(x).getMasterUserBussinessUnitBuBisCode() },${ rq_roles.get(x).getProjectPrIdProject() },'${ rq_roles.get(x).getUserName().replace(" ","-") }','${ rq_roles.get(x).getUnitName().replace(" ","-")  }','${ rq_roles.get(x).getProjName().replace(" ","-")  }')>
                                        <%--<a href='#' onClick=valida('${ rq_roles.get(x).getMasterUserMuEmail() }',${ rq_roles.get(x).getMasterUserBussinessUnitBuBisCode() },${ rq_roles.get(x).getProjectPrIdProject() },'${ rq_roles.get(x).getUserName() }','${ rq_roles.get(x).getUnitName() }','${ rq_roles.get(x).getProjName() }')>--%>
