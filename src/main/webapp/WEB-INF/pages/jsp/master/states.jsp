...
<head>
    ...
    <script language="JavaScript" type="text/javascript">
        function borrar_usuario(){
            ...
            if(validaItems(this)){
                //alert("Paso 1");
                if ( confirm("Do You really want to delete the selected State?")) {
                    //alert("Paso 2");
                    //document.forma.target = "";
                    document.forma.action = '/objectboard/statesprocess';
        ...
        function nuevo_usuario(){
            document.forma.target = "";
            document.forma.action = '/objectboard/state';
        ...
        function validaItems(source) {
            ...
            if(cta==0){
                alert("There are no states selected to remove.");
                return false;
            }
            return true;
        }
        function valida(state,pais){
            document.forma.p_pais.value = pais;
...
<div id="pcoded" class="pcoded">
    ...
    <div class="pcoded-container navbar-wrapper">
        ...
        <div class="pcoded-main-container">
            <!--start ############################### page-header ###############################-->
            <jstl:set var="jsp_titleFrom" scope="page" value="States List Form"/>
                ...
                <form class="user" role="form" method="post" action="#!" name="forma" >
                    ...
                    >
                        ...
                        <tr>
                            ...
                            <th data-field="id" data-sortable="false">ID STATES</th>
                        ...
                        <jstl:forEach var="x" begin="0" end="${ rq_numFilas -1 }" step="1">
                            ...
                            <tr>
                                ...
                                <td>
                                    <jstl:out value="${ rq_states.get(x).getId_state() }">Lost Value</jstl:out>
                                    <input type='hidden' value='<jstl:out value="${ Math.toIntExact(rq_states.get(x).getId_state()) }"/>'
                                           name='p_cual_s<jstl:out value="${ f }"/>' />&nbsp;
                                </td>
                                <td>
                                    <jstl:out value="${ rq_states.get(x).getState_name() }">Lost Value</jstl:out>
                                </td>
                                <td>
                                    <jstl:out value="${ rq_states.get(x).getCountry_name() }">Lost Value</jstl:out>
                                    <input type='hidden' value='<jstl:out value="${ Math.toIntExact(rq_states.get(x).getId_country()) }"/>'
                                           name='p_cual_p<jstl:out value="${ f }"/>' />&nbsp;
                                </td>
                                <td>
                                    <a href='#' title="Edit" onClick=valida(${ rq_states.get(x).getId_state() },${ rq_states.get(x).getId_country() })>
                    ...
                    <input name='p_pantalla' type='hidden' value='states' />
