 <?php
	...
	function get_insert($table, $columns, $values, $duplicado=''){
		$tmp="INSERT INTO $table ($columns) VALUES ($values)";
		if($duplicado!=''){
			$tmp.=" ON DUPLICATE KEY UPDATE $duplicado";
		}
		return $tmp;
	}
	...
	function get_select($table, $columns, $where='', $order='', $limit='',$agrupa=''){
	...
	//-SELECT- $columns=get_commas(...) o '*'   $where=get_mult_set(...) o get_simp_set(...)
	function get_select_new($table, $columns, $where='', $agrupa='', $order='', $limit=''){
		$tmp = "SELECT $columns FROM $table";
		if($where!=''){
			$tmp.=" WHERE $where";
		}
