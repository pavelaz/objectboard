...
import com.psg.objectboard.model.own.ownsEntity.classDAO.CommentsDAO;
...
@WebServlet(name = "CommentProcessServlet", urlPatterns = "/commentprocess")
    ...
    protected void processRequest(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException, SQLException {
        ...
        switch(acciones){
            ...
            case "save":
                ...
                if(request.getParameter("p_refe")!=null) {
                    refe = request.getParameter("p_refe");
                }
        ...
        try{
            ...
            if (acciones.equals("create")){
                ...
                cvo.setCommentDescription(name);
                cvo.setCommentType(type);
                cvo.setCommentReference(refe);
                cdo.insertCommentsDAO(cvo,con);
            }else{
                if (acciones.equals("delete")){
                    cvo.setResult(true);
                    for(int x=0 ; x < cual_1.size();x++){
                        if (cvo.getResult()){
                            cvo.setResult(cdo.deleteCommentsDAO(Integer.parseInt(company_number),
                                                                    cual_1.get(x),
                                                                    con));
                        }
                    }
                    ...
                    cvo.setBussinessUnitBuBisCode(Long.parseLong(company_number));
                    cvo.setCommentCode(Long.parseLong(code));
                    cvo.setCommentDescription(name);
                    cvo.setCommentType(type);
                    cvo.setCommentReference(refe);
                    cdo.updateCommentsDAO(cvo,con);
                }
            }
